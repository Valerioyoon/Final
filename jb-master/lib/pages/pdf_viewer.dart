...
class _PdfViewer extends State<PdfViewer> {
  ...
  void initState() {
    ...
    if(box.read("recentList") == null){
      recentList = [widget.pdfPath];
      box.write("recentList", recentList);
      print("[QWER] first recent");
    }
    else{

      box.read("recentList").forEach((e)=>recentList.add(e));

      if(recentList.contains(widget.pdfPath)){
        print("[QWER] already in recent");
        recentList.remove(widget.pdfPath);
        recentList.add(widget.pdfPath);
        box.write("recentList", recentList);
      }
      else{
        print("[QWER] not in recent");
        recentList.add(widget.pdfPath);
        box.write("recentList", recentList);
      }
    }

    print("[QWER] recent : ${recentList.toString()}");
